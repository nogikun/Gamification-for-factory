...
application_idからreviewee_id/event_idへの変換が正しく動作し、
レスポンスで再びapplication_idが正しく返されることを確認する。
"""
import requests
import json
import uuid
...
def test_review_creation_with_conversion():
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            print(f"❌ レビュー作成失敗: {response.status_code}")
...
def test_review_list_api():
    ...
    try:
        response = requests.get(f"{API_BASE_URL}/reviews", timeout=30)
        print(f"レスポンスステータス: {response.status_code}")
        ...
        if response.status_code == 200:
            ...
            print(f"❌ レビュー一覧取得失敗: {response.status_code}")
            print(f"エラー内容: {response.text}")
            return None
